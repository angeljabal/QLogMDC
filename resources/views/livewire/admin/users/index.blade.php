<div>
    <div x-data="{ deleteModal: false}" :class="{'overflow-y-hidden': deleteModal}">
                                    ...
                                    @foreach ($profiles as $profile)
                                        <tr class="bg-white">
                                            ...
                                            <td class="hidden px-6 py-4 whitespace-nowrap text-sm text-gray-500 md:block">
                                                ...
                                                <button wire:click="confirmItemDeletion( {{ $profile->user->id}})" wire:loading.attr="disabled">
                                        ...
                                        {{-- <x-modal>
                                            <x-slot name="title">
                                                {{ __('Delete User - ') . $profile->user->name }}
                                            </x-slot>
                                            <x-slot name="content">
                                                {{ __(' Are you sure you want to delete this user?') }}
                                            </x-slot>
                                            <x-slot name="footer">
                                                <button  class="p-2 bg-gray-500 text-white hover:bg-gray-600 hover:text-gray-200 rounded-md" @click="deleteModal = false">
                                                    {{ __('Cancel') }}
                                                </button>
                                        
                                                <button class="p-2 bg-red-500 text-white hover:bg-red-600 rounded-md" wire:click="deleteUser({{$profile->user->id}})" >
                                                    {{ __('Delete') }}
                                                </button>
                                            </x-slot>
                                        </x-modal> --}}
    ...
    <x-confirmation-modal wire:model="confirmingUserDeletion" wire:model.defer="name">
        <x-slot name="title">
            {{ __('Delete User - ') . $name}}
        </x-slot>
        ...
        <x-slot name="footer">
            <button class="p-4 m-1 text-sm text-white bg-gray-400 rounded-md hover:bg-gray-500" wire:click="$set('confirmingUserDeletion', false)" wire:loading.attr="disabled">
                {{ __('Cancel') }}
            </button>
 
            <button class="p-4 m-1 text-sm text-white bg-red-400 rounded-md hover:bg-red-500" wire:click="deleteUser()" wire:loading.attr="disabled">
                {{ __('Delete') }}
            </button>
        </x-slot>
